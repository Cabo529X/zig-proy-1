...
pub const Sprite = struct {
    ...
    patrol_dx: f32 = 0, // solo el dron: eje y sentido de la patrulla
    patrol_dy: f32 = 0,
...
pub const World = struct {
        ...
        while (y < self.height) : (y += 1) {
            ...
            while (x < self.width and x < @as(i32, @intCast(row.len))) : (x += 1) {
                ...
                switch (row[@intCast(x)]) {
                    ...
                    'r' => self.addSprite(.{ .x = cx, .y = cy, .kind = .drone, .patrol_dx = 1 }),
    ...
    /// Avanza animaciones, patrullas, puertas y pickups. Devuelve el evento mas
